...
import { 
  Mic, MicOff, Globe, X, Languages, 
  Share2, BookOpen, LogIn
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      if (err.message.includes('permission')) {
        setError('Missing permissions. Make sure you are logged in.');
      }
    });
  }, [sessionId, user]);
  ...
  const login = async () => {
    try {
      ...
      setError('Login failed. Check internet connection and popups.');
      return null;
    }
  };
  ...
  // Audio recording — 8-second chunks sent to Gemini for transcription
  useEffect(() => {
    ...
    const startRecording = async () => {
      try {
        ...
        setError('Mic denied. Open in a new tab if testing inside an iframe.');
      ...
      reader.onloadend = async () => {
        ...
        const result = await genAI.models.generateContent({
          ...
          contents: [{
            parts: [
              {
                text: `Transcribe speech verbatim. Context: Medical Engineering conference (University of Utah / Moscow). ` +
                      `Speaker language: ${activeLang === 'en' ? 'English' : 'Russian'}. ` +
                      `Preserve technical terms exactly (e.g. ECMO, dialysis, biosensor). ` +
                      `Return transcription text only. If silence or no speech, return [SILENCE].`
              },
  ...
  const handleTranslate = async (text: string, manualSource?: 'en' | 'ru') => {
    if (!sessionId || !user) return;
    const source = manualSource || activeLang;
    try {
      const prompt = `You are an expert simultaneous interpreter at a biomedical engineering conference (University of Utah, 2026). ` +
        `Participants: Russian-speaking medical engineers/doctors from CIS countries and English-speaking American researchers. ` +
        `Domain: cardiology, ECMO, dialysis, bioelectronics, nanotech, biosensors, medical devices.\n\n` +
        `Translate the following ${source === 'en' ? 'English' : 'Russian'} text to ${source === 'en' ? 'Russian' : 'English'}.\n\n` +
        `STRICT RULES:\n` +
        `1. Maintain exact logical polarity. A negative statement must never become positive. "not stable" must translate as a negative in the target language.\n` +
        `2. Use precise medical/engineering terminology.\n` +
        `3. Output natural spoken language, not written prose.\n` +
        `4. No explanations or translator notes.\n\n` +
        `Input (${source}): "${text}"\n\n` +
        `Return JSON only: { "originalCleaned": "...", "translated": "..." }`;
      ...
      setError('Translation error. Try shorter sentences.');
    }
  };
  ...
  const lookup = async (word: string) => {
    ...
    try {
      const prompt = `Medical engineering dictionary entry for "${word}". ` +
        `Context: Biomedical engineering conference (cardiology, ECMO, dialysis, nanotech, biosensors). ` +
        `Return JSON: { "word": "...", "translation": "[to other language]", "definition": "[1-2 sentence clinical definition]", "examples": ["...", "..."], "grammar": "[part of speech]" }`;
      const result = await genAI.models.generateContent({
        model: TRANSLATE_MODEL,
        contents: prompt,
    ...
    <div className="flex flex-col h-[100dvh] bg-[#080808] text-white overflow-hidden selection:bg-amber-500/30">
      ...
      <header className="p-4 border-b border-white/5 bg-[#0a0a0a] flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-2">
          {error && (
            <div className="text-[10px] text-red-400 font-bold px-3 py-1 bg-red-500/10 rounded-full animate-pulse">
              {error}
            </div>
          )}
          {sessionId && (
          ...
          {user
            ? <img src={user.photoURL!} className="w-7 h-7 rounded-full border border-white/10" alt="avatar" />
        ...
        {!sessionId ? (
          <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-xl flex items-center justify-center p-8">
            <div className="max-w-xs w-full text-center space-y-6">
              ...
              <p className="text-zinc-500 text-[11px] leading-relaxed">Create a shared live transcript for medical engineering interpretation.</p>
              <button
                onClick={createSession}
                disabled={isLoading}
                className="w-full py-4 bg-white text-black font-black uppercase text-xs rounded-2xl tracking-widest hover:bg-amber-400 transition-colors disabled:opacity-50"
              >
          ...
          <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-xl flex items-center justify-center p-8">
            <div className="max-w-xs w-full text-center space-y-6">
              ...
              <p className="text-zinc-500 text-[11px] leading-relaxed">Authenticate to join this secure medical session.</p>
              <button
                onClick={login}
                ...
                {messages.map(m => (
                  <div key={m.id} className="group">
                    ...
                    <div className="text-[8px] text-zinc-700 font-bold uppercase mt-1">
                      {m.userName} &bull; {m.timestamp?.toDate?.().toLocaleTimeString() || '...'}
                    </div>
                  </div>
                ))}
                <div ref={scrollRefRu} />
      ...
      <footer className="p-4 bg-[#0a0a0a] border-t border-white/5 flex flex-col gap-4 pb-[env(safe-area-inset-bottom,1rem)]">
        <div className="flex gap-2 w-full max-w-2xl mx-auto items-center">
          <input
            placeholder={`Type ${activeLang === 'en' ? 'English' : 'Russian'} here...`}
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                handleTranslate((e.target as HTMLInputElement).value);
                (e.target as HTMLInputElement).value = '';
              }
            }}
            className="flex-1 bg-zinc-900 border-none text-xs px-4 py-3 rounded-xl focus:ring-1 focus:ring-amber-500 text-zinc-300 outline-none"
          />
          <div className="flex bg-zinc-800 rounded-xl p-1 border border-white/5">
            <button
              onClick={() => setActiveLang('en')}
              className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${activeLang === 'en' ? 'bg-amber-500 text-black shadow-lg' : 'text-zinc-500'}`}
            >EN</button>
            <button
              onClick={() => setActiveLang('ru')}
              className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${activeLang === 'ru' ? 'bg-amber-500 text-black shadow-lg' : 'text-zinc-500'}`}
            >RU</button>
          </div>
          <button
            onClick={() => setIsRecording(!isRecording)}
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-2xl ${
              isRecording ? 'bg-red-500 animate-pulse text-white' : 'bg-white text-black hover:rotate-6 active:scale-90'
            }`}
          >
          ...
          >
            <div className="bg-[#111] border-t border-white/10 rounded-t-[2.5rem] p-8 pb-12 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] pointer-events-auto max-w-2xl mx-auto w-full">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setSelectedWord(null)} className="p-2 text-zinc-500 hover:text-white">
                  <X size={24} />
                </button>
              </div>
